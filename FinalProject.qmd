...
First, we will fit a linear model to the data for simplicity. We bucket the data by  `rurality` that is a factor with three levels: urban, suburban, and rural.
...
rurality_levels <- c("Urban", "Suburban", "Rural", NA)
model_coefficients <- list()

for (rural_level in rurality_levels) {
  filtered_df <- df_predictors %>%
    filter(!is.na(rate_65_plus) & !is.na(rate_85_plus)) %>%
    filter(rurality == rural_level) %>%
    mutate(rate_18_plus = totalpop18plus / totalpopulation)
    select(
      #-rate_65_plus,
      -rate_85_plus,
      -rurality,
...
UPDATE dynamic summary of coefficients below
